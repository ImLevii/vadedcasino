...
function Avatar(props) {
  ...
  function useDefaultAvatar(event) {
    ...
    event.currentTarget.src = '/assets/icons/default-avatar.svg'
  }
        ...
        <>
            <div class={'avatar ' + (props?.dark ? 'dark ' : '') + (levelToColor(getUserLevel(props?.xp)))} style={{ width: props.height + 'px', height: props.height + 'px', 'min-width': props.height + 'px' }}>
                {props?.id === '?' ? (
                  ...
                  <img src={props?.id ? `${import.meta.env.VITE_SERVER_URL}/user/${props?.id}/img` : '/assets/icons/default-avatar.svg'} alt='' onError={useDefaultAvatar} style={{ height: props?.id ? ((props.height - 2) + 'px') : ((props.height - 2) + 'px') }}/>
                )}
            </div>
